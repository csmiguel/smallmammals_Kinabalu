#' MLPE model
#' 10.1002/sim.3429
#' This function runs a maximum likelihood population effects mixed effects
#' model given two pairwise distance matrices. The code for this function is
#' based on \code{\link[ResistanceGA]{MLPE.lmm}}.
#'
...
mlpe <- function(y, x, REML = FALSE) {
  ...
  dat <- dplyr::mutate(dat, x = (x - mean(x)) / sd(x),
                            y = (y - mean(y)) / sd(y))
  colnames(dat) <- c("id1", "id2", "x", "y")
  # prepare model
  mod_list <- list(
    "full" = lme4:::lFormula(y ~ x + (1 | id1), data = dat, REML = REML),
  ...
  # calculate R2
  MODs$r2 <-
    r2glmm::r2beta(lme4::lmer(y ~ x + (1 | id1), data = dat, REML = REML),
                   partial = FALSE, method = "kr")
  # return full and null model
  MODs
}
